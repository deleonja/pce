...
\begin{itemize}
\item Quizá en este punto hablar un poco de decoherencia de $N$ qubits, como 
solo para motivar los PCEs y decir que se va a ligar al final. 
...
$\alpha_{1,\cdots,N}$ is varied from $0$ to $3$\cpnote{Quizá se cambia
cuando hayamos escrito lo de un qubit}. 
...
A graphical representation for 1, 2 and 3 qubits may be introduced, being the
two qubit case what has proved more useful. Consider a $N$ dimensional 
grid, with $4^N$ places. Each place has $N$ integer coordinates, ranging
...
for two qubits, we exemplify them in \fref{fig:two:qubit:examples}. Notice
than in both cases we drop the label $\alpha$ for simplicity, as it will 
remain unchanged throughout this text. 
...
\end{tikzpicture}
\end{center}
	\caption{\cpnote{Aca meter la figura, JA} In (a) we introduce the positions of the single 
qubit diagrams, so that each square corresponds to a single 
$\tau_{\alpha}$, $\alpha=0,\cdots,3$. The diagrams in 
(b), (c) and (d) correspond to the identity map, \cpnote{bit flip ao algo asi}, and 
total depolarization, respectively. In (e) we show a map that erases the two components
$r_1$ and $r_2$ and thus does not correspond to a quantum operation.}
	\label{fig:one:qubit:examples}
\end{figure} % }}}

\begin{figure} % {{{
	\caption{\cpnote{Aca meter la figura, JA} In (a) we introduce the
positions of two qubit diagrams. The diagrams in 
(b) corresponds to a quantum channel that \cpnote{ver con JA que ejemplo ponemos}, 
