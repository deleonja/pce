...
the non-local feature of quantum theory. 
...
map is not always positive. If, for all dimensions of the ancilla, such extension
results in a positive map, we say that the original map is completely
positive~\cite{}.  Quantum channels are required to be completely positive
...
\pce{} maps are defined as those quantum channels\janote{mapas PCE no son necesariamente
canales cuánticos} that \textit{nullify} Pauli strings. In the case of one qubit, they consist of maps nullifying the averages $r_\alpha:=\langle \sigma_\alpha \rangle=\tr \left( \rho \sigma_\alpha \right)$ with $\alpha=1,2,3$. The three components $r_\alpha$ form the well-known \textit{Bloch vector} $\vec r$, describing one qubit density matrices,
\begin{equation}
