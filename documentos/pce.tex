...
and finish with the definition of PCE maps, some generalities \afnote{...and some generalities. (?)}. We further 
...
One first exploits \janote{the} \sout{an} isomorphism that maps a channel $\mcE$ to the state
...
by exploring the single-qubit scenario and then we treat the $N$\janote{-}qubit case.
% }

The most general single\janote{-}qubit density matrix can be written as
\begin{equation}
...
\eref{EqPCE}, as \janote{single-qubit} \textit{Pauli component erasing} (\pce{}) maps.
...
\end{align}
where trace-preserving is already imposed, \janote{and} show that five out of
...
in the diagrams that represent the single\janote{-}qubit \pce{} maps,
...
In order to present and develop the $N$\janote{-}qubit case, it is useful 
to introduce 
...
Similarly to the single\janote{-}qubit case, the density matrix $\rho$ of a system of $N$
...
0}=1$. 
It is worth noticing that, as for the single\janote{-}qubit case, not all PCE maps are
quantum operations. 
