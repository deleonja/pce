\section{PCE Maps} 
In this section we introduce the definition of Pauli component erasing maps, but first we go through several standard definitions needed for.
...
\mcE_{ij}=\tr\left( \Delta_i \mcE[\Delta_j] \right)$, are real. In particular, in this work we are interested in systems of $N$ qubits, and we use hermitian bases consisting of the so-called \textit{Pauli strings}, defined as 
\begin{equation}
...
\end{align}
where trace-preserving is already imposed ($\tau_0=1$), show that only five operations are \pce: the identity map, the completely depolarizing channel ($\rho \mapsto \one/2$); and bit, phase, and the bit-phase flip channels~\cite{chuangbook}. These channels can be pictured with one column tables showing the positions of $1$s and $0$s in the diagonal of the channel (in the Pauli basis), see~\fref{fig:one:qubit:examples}.
