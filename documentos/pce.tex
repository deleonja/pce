\section{PCE Maps} 
To introduce the family of Pauli component erasing maps we start \sout{with 
review}\janote{reviewing} basic concepts of quantum channels that will allow us to introduce some 
notation. 
...
Often, this condition is easy to test.\janote{El comentario anterior se me hace de más}
The
complete
...
\janote{Siento que la subsección comienza de golpe con 'hablemos de los PCE 
de 1 qubit' y hace falta una transición de lo de arriba a los PCE, además 
de una frase que resuma de qué hablamos en II.B. Propongo 
la siguiente reformulación:}

\janote{Now that we have discussed the main features of quantum channels
we are going to introduce the Pauli component erasing maps.}
We shall start exploring \sout{a}\janote{the} single qubit scenario 
\janote{to finish developing the $N$-qubit case}\sout{, where the most general 
density matrix can be written as}\janote{The most general 
...
\end{equation}
where $\tau_\alpha$ is either $1$ or $0$ (trace preserving requires
that $\tau_0=1$). Not every
such operation  is a quantum map; for example, collapsing the entire Bloch
ball to the $xy$ plane (setting $\tau_{1,2}=1$ and  $\tau_3=0$) is not a quantum
...
\end{tikzpicture} % }}}
\end{center}
	\caption{In (a) we introduce the positions of 
\janote{\textbf{in}(?) the diagrams that picture the single qubit \pce{} maps}
\sout{the single qubit diagrams}\janote{para que sea autocontenida la caption}, so that each square corresponds to a single 
...
$r_1$, collapsing the Bloch sphere into a disk, and thus does not correspond to a quantum operation.}
...
A Pauli component erasing (PCE) map is a map
...
% \end{split}
\end{equation}
For the operation to be trace preserving, it is required that $\tau_{\vec
0}=1$. 
...
2^N $ components equal to $1$s in its diagonal, see appendix~\sref{quantum_classical}.
