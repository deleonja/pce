\section{propuestas abajo}\label{sec:2_qubits}
{\red propuestas de CP y JA: (la democracia decidirá)}

\cpnote{Labeling and diagrams of PCE generators}

\janote{Local-action and labeling of PCE generators}
...
\cpnote{mi contra-contra propuesta}
\janote{Que lo revise la gente}


The local-action of a generator $\mcG_\valpha$ on every qubit in the 
system depends only, as its notations suggests, on the vector index $\valpha$. 
This index has a simple meaning that can be read from the graphical representation
of the channel. 
Recall the single qubit PCE generators, shown in in 
