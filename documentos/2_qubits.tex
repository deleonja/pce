...
\janote{Está cruda. Falta iterarla una vez.}
...
For this matter, it will show instructive to start discussing the generators 
$\mcG_\alpha$ of single qubit PCE quantum channels. As shown in 
...
flip channels. In the PCE diagrams picture the index $\alpha$ is associated
with the black positions except for $\mcG_0$ that has all positions black.
\janote{
Recall that, besides $\tau_0=1$ for every PCE quantum channel, $\mcG_{0,1,2,3}$
are charecterized by which $\tau$ are equal to one.
}
\janote{This will be the basis for the following discussion.}
To interpret the meaning of $\valpha$ in $\mcG_\valpha$, we consider the reduced 
density matrix of the $k$-th qubit after the action of the generator $\mcG_\valpha$
on the whole system,
\begin{align}
\tr_{\not k} \mcG_\valpha [\rho] &= 
...
where the partial trace over everything except qubit $k$ is
denoted by $\tr_{\not k}$.
\janote{In the last line of \eqref{eq:generator:individual:qubit} 
...
Therefore, $\mcG_{\alpha_k}$ acts on $\rho_k$ as 
$r_{0,\ldots,j_k,\ldots,0}\mapsto 
\tau_{0,\ldots,j_k,\ldots,0}r_{0,\ldots,j_k,\ldots,0}$.
This lets us characterize[?] every PCE generator depending on the
values of $\tau_{0,\ldots,j_k,\ldots,0}$}  
