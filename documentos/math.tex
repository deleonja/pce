...
% Intro {{{
This section is devoted to derive the conditions a Pauli map need to
satisfy to fulfill the CP condition i.e. that all the eigenvalues of the Choi matrix
associated to the channel must be non-negative. 
To do so, we first calculate and diagonalize the matrix associated with general 
Pauli maps, 
first for a single qubit and then for $N$ qubits. We then restrict to
PCEs and, for quantum channels, associate a vector space
to the set of coefficients $\{\tau_\valpha\}$ in \eref{eq:PCE_definition}, from 
which we are able to derive several important conclusions. 
\afnote{This section is devoted to derive the conditions a Pauli map need to
satisfy to fulfill the CP condition i.e. that all the eigenvalues of the Choi matrix
associated to the channel must be non-negative. 
To do so, we first calculate and diagonalize the matrix associated with general 
Pauli maps, both for a single qubit and for $N$ qubits. We then restrict to
PCEs and, for quantum channels, associate a vector space
to the set of coefficients $\{\tau_\valpha\}$ in \eref{eq:PCE_definition}, from 
which we are able to derive several important conclusions.}
% }}}
...
we chose $a$ as some later considerations [see  \eref{eq:sigma_property}]
...
$a^N$, which might be more convenient for other applications.
