...
\end{equation}
%
where the kets $\ket{\sigma_{\mu}}$ correspond to the vectorized elements of the Pauli basis and satisfy the usual relation $ \braket{{\sigma_{\mu}}}{{\sigma_{\mu'}}} = \tr \left(\sigma_{\mu}^{\dagger} \sigma_{\mu'}\right) = 2 ~\delta_{\mu\mu'}$. After some steps it is possible to show that the associated Choi matrix reads
% 
\begin{equation}
