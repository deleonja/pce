...
\begin{frame}{\enges{Density matrix}{Matriz de densidad y traza parcial}} % {{{
% \cpnote{
% The density matrix $\rho$ is an operator that represents a state in which {\it
% all} measurable information is encoded (similar to the state vector).
% % \pause
% % \item
% It is used often in open quantum systems, quantum information and statistical mechanics. 
% The measurable quantities, expected values, are calculated as $\tr \hat M \rho$
% }
\enges{The density matrix $\rho$ is an operator that represents a state}{
Una matriz de densidad $\rho$ es un operador que describe un estado, con un poco más
de generalidad que un ket.} \\[.0cm] \pause 

% \enges{Consider a quantum system 
% that is in on one of the states}{
% Consideremos que nuestro sistema cuántico está en uno de los estados}
% $|\psi_i\>$ 
% \enges{with probability}{con probabilidad}
% $p_i$ ($\{(p_i, |\psi_i\> \}_{i \in I}$).
% \enges{The corresponding \alert{density matrix} is }{
% La \alert{matriz de densidad} correspondiente es }
% \begin{equation*}
% \rho  = \sum_{i\in I} p_i |\psi_i\>\< \psi_i|.
% \label{}
% \end{equation*}
% \pause
\begin{itemize}
% \item 
% Un estado puro se puede referir a un vector en el espacio de Hilbert, 
% o a una matriz de densidad que tenga sólo una componente en la superposición. 
% \pause
\item \enges{Contains {\it all} physical information}{Contiene {\it toda} la
información física}
\pause
% \item Se usa en mecánica estadística, sistemas abiertos, información cuántica
%  porque describe ensambles
% \item Se usa en información cuántica porque describe ignorancia
% \item 
% \enges{
% The density matrix is often used in open quantum systems. 
% }{
% La matriz de densidad se usa en sistemas abiertos. 
% }
% \\[.5cm]
\item \enges{The expected value of an observable is}{El valor esperado de un observable} $\< \hat A \>_\rho = \tr \rho \hat A$
% \item 
% \enges{It is also often used in statistical mechanics and in quantum information.}{
% También se usa mucho en mecánica estadística e información cuántica. }
\item \enges{Convenient in several contexts, but introduces no new
physics}{Conveniente en varios contextos, no introduce nueva física}\\[.5cm]
\end{itemize}
\pause

\enges{In a bipartite system $\mcH_A \otimes \mcH_B$, the state
of a subsystem $\rho^A$ is related to the whole state $\rho^{AB}$ via}{
En un sistema compuesto %, $ \mcH_A \otimes \mcH_B$}, 
% $\mcH = \mcH_A \otimes \mcH_B$,
el estado de un subsistema está relacionado con el estado completo mediante}
% $ \mcH_A \otimes \mcH_B$,
% \begin{equation*}
% \rho^{AB}.
% \end{equation*}
% \pause
% \enges{The reduced density matrix for system $A$ is defined as}{El operador de
% densidad reducido para el sistema $A$ está definido como }
\begin{equation*}
% \alert{
\rho^A = \tr_B \rho^{AB}.
% }
...
\pause
\enges{The partial trace is meaningful since it predicts
the right expected values:}{La operación traza parcial tiene sentido, pues
predice apropiadamente valores esperados:}
% con $\tr_B (|a_1 b_1\>\<a_2 b_2|) = |a_1\>\<a_2| \<b_1|b_2\> $ y tiene
% sentido pues
% \enges{and we extend to other operators requiring linearity.}{
% y extendemos a otros operadores usando linealidad. }
% Esta definición tiene sentido porque 
\begin{align*}
\< M_A \>_{\rho_A} =  \< M_A \otimes \openone \>_{\rho_{AB}}.
\end{align*}
\end{frame} % }}}
...
\end{frame} % }}}
% \begin{frame}{Ejemplo: canales genéricos} %{{{ 
% 
% Un qubit acoplado fuertemente a un ambiente
% \begin{itemize}
% \item \enges{Hilbert space}{Espacio de Hilbert:} $\mcH = \mcH_\q \otimes \mcH_\env $
% \item \enges{Hamiltonian}{Hamiltoniano:} $H = V_\text{GUE}$ (sin estructura, acople fuerte)
% \end{itemize}
% 
% \enges{Solution}{Solución}:\\
% \begin{itemize}
% \item 
% \enges{Non-markovian depolarizing channel}{Canal de depolarización} $\rho \to \alpha(t) \rho + 
% \left[1-\alpha(t) \right] \openone/2$
% % \item ¡A pesar de tener 0 estructura, el canal es no-Markoviano!
% \end{itemize}
% 
% \begin{center}
% % \begin{minipage}{.49\textwidth}
% % \centerline{\footnotesize \enges{Cold state}{Estado frio} $N=12000$}
% % % \begin{center}\footnotesize \enges{Cold state}{Estado frio} $N=12000$\\[-1cm] \end{center}
% % \includegraphics[width=5.7cm, clip=true, bb=0 0 245 150]{LambdasFinalTitle}
% % \end{minipage}
% % \hfill
% \begin{minipage}{.49\textwidth}
% % \centerline{\enges{Hot state}{Estado caliente} $N=100$}
% % \begin{center}\footnotesize \enges{Hot state}{Estado caliente} $N=100$\end{center}
% \includegraphics[width=5.7cm, clip=true, bb=0 0 245 150]{identityFinal}
% \end{minipage}
% \end{center}
% \vfill \tiny \hfill 
% Phys. Rev. Lett. 107, 080404 (2011)
% \end{frame} %}}}
% \begin{frame}{Ejemplo: clasificando canales de un qubit} % {{{
% % Para poder estudiar y compartir con ud
% 
% \begin{center}
% % \raisebox{3cm}{$\rho=\frac{\openone + \vec r \cdot \vec \sigma}{2}$}
% \includegraphics[width=0.3\textwidth,viewport=70 90 310 330,clip]{../images/transformacionGeneral1.jpg}%
% \hspace{.3cm}
% \onslide<2->{
% \includegraphics[width=0.3\textwidth,viewport=70 90 310 330,clip]{../images/transformacionGeneral2.jpg}
% }
% \raisebox{0cm}{
% \onslide<3->{
% \resizebox{3.5cm}{4cm}{%
% \begin{tikzpicture}
% % \node at (0,0) {\includegraphics{simpleTetra.pdf}};
% \node at (0,0) {\includegraphics{/home/carlosp/investigacion/antiguos_proyectos/nm_static_dynamical/images/simpleTetra.pdf}};
% \node at (-1,-3.7) {$\lambda_1$};
% \node at (-2.2,3.5) {$\lambda_2$};
% \node at (-3.8,0) {$\lambda_3$};
% \end{tikzpicture}
% }
% }
% }
...
% \begin{itemize}
% \onslide<1->{
% \item Usamos la representación de la esfera de Bloch 
% $\rho \propto \openone + \vec r \cdot \vec \sigma$}
% \onslide<2->{
% \item Cualquier canal corresponde a
% $\vec r\to R\cdot \diag(\lambda_i)\cdot R' \cdot\vec r+\vec \tau$}
% % \item Las rotaciones {\tiny casi} no son importantes en este contexto
% \onslide<3->{
% \item Caracterizamos en canal con $\lambda_{1,2,3}$ fijando $\tau$.
% }
% \end{itemize}
% % Para canales unitales, donde el desplazamiento es nulo $\mcE(\openone)=\openone$
% % nos quedamos con 3 parametros, y las rotaciones que frecuentemente podemos ignorar, 
% 
% % 1. Como visualizar los canales: Estos forman un espacio vectorial? Son operadores
% % sobre el espacio de matrices de densidad. 
...
% \end{frame} % }}}
% \begin{frame}{Canales de un qubit tipo Lindblad} %{{{ 
% 
% % \begin{minipage}{.49\textwidth} % {{{
% % \begin{center}\footnotesize Reachable with P-divisible maps\\
% % \includegraphics[width=.5\textwidth]{biTetra.pdf}
% % \end{center}
% % \end{minipage} % }}}
% % \hfill
% % \begin{minipage}{.49\textwidth} % {{{
% % \begin{center}\footnotesize Entanglement breaking and divisible but not infinitely divisible
% % \includegraphics[width=.5\textwidth, viewport=0 0 230 218,clip=true]{ininTetra.pdf}
% % \end{center}
% % \end{minipage} % }}}
% % 
% % \phantom{x}\vspace{.0cm}
% 
% \begin{minipage}{.49\textwidth} % {{{
% \begin{center}% \footnotesize Linblad maps\\
% \includegraphics[width=.5\textwidth,viewport=0 0 230 218,clip=true]{skewTetra.pdf}
% \end{center}
% \end{minipage} % }}}
% \hfill
% \begin{minipage}{.49\textwidth} % {{{
% \begin{center}%\footnotesize Time dependent Lindblad maps
% \includegraphics[width=.5\textwidth, viewport=0 0 230 218,clip=true]{CPTetra.pdf}
% \end{center}
% \end{minipage} % }}}
% 
% \vspace{1cm}
% \begin{itemize}
% \item Podemos encontrar que tipo de procesos podemos implementar con 
% relajación  (ecuación de Lindblad)
% \item Podemos encontrar que tipo de procesos podemos implementar con 
% relajación dependiente del tiempo (ecuación de Lindblad dependiente del tiempo)
% % \item Caracterizamos algebraica y geométricamente el conjunto \LDiv{} (y los otros)
% % \item Podemos trazar rutas para implementar mapas de acuerdo a las restricciones experimentales
% \end{itemize}
% \end{frame} % }}}
% }}}
\end{document}
...
Intro larga de canales cuanticos
La idea
Matriz de densidad
Canal
completa positividad, isomorfismo de Choi, diagonalizacion.
ejemplos de como no funciona
Deficiion abstracta
Ejemplos de un qubit, 
Rotaciones, 
Cosas que no son calanes pero si son los PCEs.
Ejemplo de 1 qubit
Definicion de PCE, un qubit
Definicion de PCE, n qubits, para esto, quizá necesito irme a la definicion
de las sigmas para n qubit.s 
Diagonalizacion de la matriz de Choi. 

Planteamiento de coso como un espaico vectorial. Inversion de la matriz A, 
Explicacion del espacio vectorial en cuestion. 
Conescuencias, como el numero de elementos del bicho ese 
cuantos canales hay
regla del arco iris.

Louego, los generadores. 
* Explicacion intuitiva
* Perspectiva desde los espacios vectoriasles
* Simetrías presentes
* Clasificacion de los generadores
* 

Operadores de lKraus para decoherencia
* Casos mas generales para la decoherencia. 



Ejemplos de dos qubits.
