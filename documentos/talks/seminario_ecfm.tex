...
{\bf Geometría de los canales cuánticos: canales cuánticos PCE y de Weyl}% this is the long title
%
% author
\author[José Alfredo de León]% this is the short name
{José Alfredo de León \\
Agregar al resto de autores y figuritas}
...
\begin{frame}[t]{\normalsize Estados de un sistema cuántico de dos niveles}

\vspace*{-3mm}
\begin{minipage}{.49\textwidth}
\small
$$\ket{\psi}=\cos\qty(\frac{\theta}{2})\ket{0} + e^{i\phi}\sin\qty(\frac{\theta}{2})\ket{1}$$
\begin{center}
\includegraphics[width=5.5cm]{../images/bloch_general}
Estados puros
\end{center}
\end{minipage}
\hfill
...
\small
$$\rho = \mqty(1+r_z&r_x-ir_y\\r_x+ir_y&1-r_z)$$
\begin{center}
\includegraphics[width=5.5cm]{../images/bloch_rho}
Estados puros \textbf{y mixtos}
\end{center}
\end{minipage}
...
\begin{frame}{\normalsize
En general, la evolución de un sistema no es unitaria}

\hspace*{-.5cm}
\begin{minipage}{\textwidth}
\vspace*{-1mm}
\begin{center}
\includegraphics[width=12cm]{../images/evolucion_unitaria}

\includegraphics[width=12cm]{../images/evolucion_no_unitaria}
\end{center}
\end{minipage}
\end{frame}
...
\begin{frame}{\normalsize Algunas proyecciones de la esfera de Bloch modelan decoherencia}{¿Cualquier proyección es físicamente posible de hacer?}
\begin{center}
\resizebox{0.9\textwidth}{!}{
\begin{tikzpicture}
\node at (-10,5)  {%
\includegraphics[width=4.5cm]{../images/channel_00}%
};
\draw [-stealth,line width=7pt] (-7.5,4.5) -- (-5.5,4.5);
\node at (-6.5,6.3) {\color{green} \Huge\cmark};
\node at (-6.5,5.6) {\LARGE evolución};
\node at (-6.5,5.1) {\LARGE física};
\node at (-3,5)  {%
\includegraphics[width=4.5cm]{../images/channel_02}%
};
%
%
\node at (-10,  0.5)  {%
\includegraphics[width=4.5cm]{../images/channel_03}%
};
\node at (-6.5,1.8) {\color{red} \Huge\xmark};
\node at (-6.5,1.1) {\LARGE no es evolución};
\node at (-6.5,0.6) {\LARGE física};
\draw [-stealth,line width=7pt] (-7.5,0) -- (-5.5,0); 
\node at (-3,  0.5)  {%
\includegraphics[width=4.5cm]{../images/channel_04}%
};
\end{tikzpicture}
}
\end{center}
...
\begin{frame}
Consideremos el mapeo $\Phi$ que proyecta la esfera de Bloch al plano xy
\begin{center}
\includegraphics[width=\textwidth]{../images/cp.png}
\end{center}
\end{frame}

\begin{frame}
\begin{center}
\includegraphics[width=.7\textwidth]{../images/dibujo}
\end{center}

Para un sistema de $N$ partículas de 
dos niveles, \alert{¿cuáles son las proyecciones de la hiperesfera de Bloch
que sí describen evoluciones físicas?}
\end{frame}
...
\begin{frame}{Canales cuánticos PCE: son subgrupos}\scriptsize
\vspace*{12pt}
\onslide<1->{
Si $\tau_{pq}=\tau_{p'q'}=1$ de un canal cuántico PCE ($p,q\ne p',q'$), 
entonces $\tau_{p+p',q+´q'}=1$.\\[8pt]}
\onslide<2->{\scriptsize 
\textbf{\textit{Demostración}}: consideremos}
\begin{align}
\onslide<2->{
\lambda_{\mu \nu}=\frac{1}{2}\sum_{p,q=0}^1(-1)^{\mu p+\nu q\Mod{2}}\tau_{pq}}
\onslide<3->{
\hspace*{2pt}&\Leftrightarrow\hspace*{2pt} 
\tau_{pq}=\frac{1}{2}\sum_{\mu,\nu}(-1)^{-\mu p-\nu q\Mod{2}}\lambda_{pq},\label{eq:exp:1} \\}
\onslide<4->{\sum_{\mu,\nu}\lambda_{\mu\nu}&=2.\label{eq:exp:2}\\[-15pt]}\nonumber
\end{align}
%\onslide<4->{De aquí en adelante utiliremos implicítamente la suma modulo 2 
%de los índices.}
\onslide<5->{Restamos \eqref{eq:exp:2} de la expresion 
\eqref{eq:exp:1}, 
...
\onslide<5->{
\sum_{\mu,\nu}\lambda_{\mu\nu}\qty[(-1)^{-\mu p-\nu q\Mod{2}}-1]=0}
\onslide<6->{\hspace*{10pt}\Leftrightarrow \hspace*{10pt}
(-1)^{-\mu p}&=(-1)^{\nu q},\quad \forall \mu,\nu\vert \lambda_{\mu\nu}\ne 0.}
\end{align*}
\onslide<6->{
Similarmente para $p',q'$.}}
\onslide<7->{Luego,
\scriptsize{
\begin{align*}
...
\onslide<8->{
\alert{\noindent Es decir, los índices forman un grupo}
\begin{align*}
(p,q)+(p',q')=(p+p',q+q').
\end{align*}}
\end{frame}
