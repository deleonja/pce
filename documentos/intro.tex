...
for multi-particle systems~\cite{Horodecki}, \janote{that is why} an important part
...
in characterizing quantum channels is that, like for quatum states, 
...
matrices)~\cite{KIMURA2003339,Lawrence2002}, \janote{hence} the resulting maps are unital and
...
From now on we refer to these maps as \textit{Pauli component erasing} (\pce{})
maps. We
derive necessary and sufficient conditions for such mappings to be completely
positive \janote{and} trace\janote{-}preserving \janote{(CPTP)}, thus, showing which idempotent operations are valid
...
section~\ref{sec:kraus}, as well \janote{as} Kraus operators of the generators.
% 
To finish, we conclude \janote{and discuss future perspectives} in section~\ref{sec:conclusions}.
% }}}
