...
resource for a wide variate \afnote{(variety)} of tasks that include teleportation~\cite{PhysRevLett.70.1895},
...
generalization of the Bloch sphere \afnote{seria bueno mencionar en que sentido es dicha generalización.}. 
...
frase, de lujo}
\afnote{In addition, these channels describe the decoherence process in a similar way as the aforementioned
ones for the single-qubit case.}
We call such maps \textit{Pauli component erasing} (\pce{}) maps \afnote{From now on we refer to them as \textit{Pauli component erasing} (\pce{}) maps}. We
derive necessary and sufficient conditions for such mappings to be completely
positive trace preserving \afnote{(CPTP)}, thus, showing which idempotent operations are valid
quantum channels (\pce{} channels). Additionally, we characterize the algebraic
properties of the set of \pce{} channels, showing that \afnote{it} is equipped with a
...
\pce{} maps as finite vector spaces \afnote{The section~\ref{sec:math} is devoted to present an analytical diagonalization of the the Choi matrix for
arbitrary \pce{} maps, and to provide a characterization of completely positive maps by interpreting
\pce{} maps as finite vector spaces}. In section~\ref{sec:generators} we study
generators of the semigroup structure of the set of \pce{} channels, and use
...
section~\ref{sec:kraus}, as well Kraus operators of the generators.}
% 
\afnote{No entendí que se quiere decir con ``as well Kraus operators of the generators"} To finish, we conclude in section~\ref{sec:conclusions}.
% }}}
