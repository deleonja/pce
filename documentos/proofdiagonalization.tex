...
\ddnote{Alejo, por fa revise q no se me fue un typo con el asunto de mod 2, ya corregí toda la notación y removi los otros comentarios, que ya fueron aclarados en la reunion y trataban de lo mismo} \afnote{Listo, todo en orden.}
...
the sake of clarity, we will use \afnote{we use} Latin symbols for binary indices, and
...
\begin{equation}
 ...
 \label{Choi_Pauli_1q}
\end{equation}
Back to the Eq. \ref{Choi_a1}), let us apply the index relabeling $k\to j+
...
since $ (-1)^{j+(j+ k (\text{mod} 2))} = (-1)^k $ \ddnote{Alejo, por fas checa esto, se me hace curioso que el modulo 2 solo este solo en la segunda suma del exponente} \afnote{Efectivamente solo tiene el mod2 en la segunda, sin embargo da exactamente lo mismo incluso si no existiera el mod2, pues la base es la misma (-1). En el caso de Weyl si hay que tener extremo cuidado con eso.}. To continue, we 
...
\end{array}
\right),
\end{equation}\janote{esta matriz ya está en el cuerpo principal del artículo. Sugiero quitar de acá.}
...
\ddnote{Poner atencion a lo siguiente}
{\red[viejo y mal]The tensor product of Pauli matrices, in vector form,  will be denoted by 
...
{\blue [nuevo y correcto y breve, la condicion de ortogonalidad no se usa explicitamente y es trivial]
