...
the sake of clarity, we will use Latin symbols for binary indices, and
...
compactly as
$\sigma_{k l }=\sum_{j=0}^{1} (-1)^{j k }\dyad{j}{j\oplus l }$. In vectorized form
\begin{align}
\kket{\sigma_{k l }}=\sum_{j=0}^{1} (-1)^{j k }\kket{j,j\oplus l },\\
\janote{
\kket{\sigma_{k l }}=\sum_{j=0}^{1} (-1)^{j k }\ket{j,j+l} \text{ (mod 2)},
}
\label{CompToPau}
\end{align}
and its inverse relation \ddnote{falta definir que es el $\oplus$ en este contexto, y no solo eso, sino quienes son los vectores del lado derecho, \ie{} $\kket{k,l}$}
\janote{yo creo que es mejor dejar el $\oplus$ para la suma de Francois y usar otro símbolo para suma módulo 2. Además, creo que es mejor escribir $\ket{j,j+l'} \text{ (mod 2)}$ 
para que sea claro que esos son vectores canónicos (son los canónicos, verdad?)},
\afnote{La base usada es la computacional, pero en esta parte no sabria como nombrarla. Tal vez ``Vectorized computational basis"? En relación al símbolo $\oplus$, creo que es mejor especificar dentro del texto que esta es una suma modulo 2, y en la parte de François que es suma módulo 4, esto con el fin de no sobrecargar la notación dentro de las ecuaciones.}
\begin{equation}
\kket{k,k\oplus l}=\frac{1}{2}\sum_{j=0}^{1} (-1)^{jk}\kket{\sigma_{jl}}.
\label{PauToComp}
\end{equation}
...
\begin{equation}
 ...
 \label{Choi_Pauli_1q}
\end{equation}
Back to the Eq. \ref{Choi_a1}), let us apply the index relabeling $k\to j\oplus
k$, then the Choi matrix reads
\begin{equation}
 \mathcal{D} =
    \frac{1}{2}\sum_{jk lm} \tau_{ lm} (-1)^{ l k} 
        \ddyad{j,j\oplus k}{j\oplus m,j\oplus m\oplus k},
\nonumber
% \label{Choi_a1}
\end{equation}
% thanks to a simplification in the argument of the power of $-1$. To continue, 
since $ (-1)^{j+j\oplus k} = (-1)^k $. To continue, we 
...
\end{equation}
or more compactly $\vec{\lambda} =(1/2) H \otimes H \vec{\tau}$, where 
...
\eref{eq:lambda:is:tau:appendix} can be redone, but with vector indices instead of binary scalars. 

The tensor product of Pauli matrices, in vector form,  will be denoted by 
$\kket{\sigma_{\vec{k}\vec{l} }} = \kket{\sigma_{k_1l_1}}\otimes \cdots \otimes \kket{\sigma_{k_Nl_N}}$
and fulfill the orthogonality relation 
$\bbrakket{\sigma_{\vec{k}\vec{l} }}{\sigma_{\vec{k}'\vec{l}'}} = 
2^N \delta_{\vec{k}\vec{k}'}\delta_{\vec{l}\vec{l}'}$. 
% \begin{equation}
...
\end{equation}
The generalization of Eqs.~(\ref{CompToPau}) and (\ref{PauToComp}) read 
\ddnote{corregir el angulo doble y tambien el oplus}
\begin{align}
\kket{\sigma_{\vec{k}\vec{l} }}  & = 
   \sum_{\vec{j}} (-1)^{\vec{j}\cdot\vec{k}} \kket{\vec{j},\vec{j}\oplus \vec{l}},\\
%
 \kket{\vec{l},\vec{l}\oplus \vec{n}} & = 
...
the map, $\hat{\mathcal{E}}_N $ in the $N$-qubits computational basis as
\begin{align}
\hat{\mathcal{E}} 
% & = \frac{1}{2}\sum_{l m=0}^{1} \tau_{l m} \ddyada{\sigma_{l m}} 
& = \frac{1}{2}\sum_{\vec j\vec k\vec l \vec m} 
     \tau_{\vec l \vec m} (-1)^{\vec l (\vec j+\vec k)} 
     \ddyad{\vec j,\vec j\oplus \vec m}{\vec k,\vec k\oplus \vec m}.
...
$\hat{\mathcal{E}}_N$ to obtain the associated Choi matrix, 
...
% \lambda_{jk}=\frac{1}{2}\sum_{l m } (-1)^{jl +k m } \tau_{l m },
\end{equation}
or more compactly $\vec{\lambda} = (H\otimes H/2)^{\otimes N} \vec{\tau}$. 
Again, we prefer to reorganize the indices to be able to write
\begin{equation}
\lambda = \frac{1}{2^N} A \vec\tau, 
\end{equation}
where 
$\san=a^{\otimes N}$. 
% \begin{equation}
