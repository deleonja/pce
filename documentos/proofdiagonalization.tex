...
reserve the Greek ones for quaternary ones \afnote{(and
...
\end{align}
\afnote{Creo que es mejor dejar el tau dentro del parentesis para dejar implícito que está sumando también.}
...
that both matrices ($\sa$ and $H \otimes H$) are equivalent up to a permutation of rows. In other words
this operation corresponds to a relabeling in the eigenvalues $\lambda_{mn}$.
% }}}
\subsection{$N$ qubits} % {{{
...
\end{equation}
The generalization of \eref{CompToPau} and \eref{PauToComp} read \afnote{eq. $\to$ Eq.}
\begin{align}
...
\begin{equation}
\lambda_{\vec{j}\vec{n}}= \frac{1}{2^{N}}\sum_{\vec{l}\vec{m}} 
    (-1)^{\vec{j}\cdot\vec{l}+\vec{k}\cdot\vec{m}}
    \tau_{\vec{l}\vec{m}} 
\end{equation}
\afnote{Aqui los vectores $l$ y $m$ en el exponente estan cambiados}
