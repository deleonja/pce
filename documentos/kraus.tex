...
\textit{implements} \cpnote{porque en italicas? Me cae gordo} the PCEG $\mcG_{\vec \alpha}$ when $t\to \infty$,
\begin{align}
...
\begin{equation}
\mcL=\frac{\gamma_{(0,3)}\left(\sigma_{(0,3)}\rho\sigma_{(0,3)}-\rho \right)}{2}
   +\frac{\gamma_{(3,3)}\left(\sigma_{(3,3)}\rho\sigma_{(3,3)}-\rho
\right)}{2},
\end{equation}
\cpnote{Aca cambio la notacion, no se deberia poner L[rho] ?}
...
We now show that PCE channels can also be described with \textit{simple
collision models}~\cite{Ziman2010} \cpnote{can also be implemented can result 
from collision models o ago asi. No cre oqeu uno describa un PCE con un modelo
de colision}. First, observe that PCEGs can
be implemented using an unitary over the system and an
ancilla~\cite{Stinespring2006}\cpnote{No creo qeu la cita vaya acá}. Since PCEG
