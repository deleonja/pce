...
PCE channels can be seen as the fixed points of some decoherence process. We will show this first for PCEGs. Consider the following dynamical process that \textit{implements} the PCEG $\mcG_{\vec \alpha}$ when $t\to \infty$,
\begin{align}
...
Since PCEGs commute, we can describe easily any other PCE channel as a fixed point of a decoherence process. For them, the Lindblad generator is simply the sum of the generators of the corresponding generators, thus, the Lindblad operators are simply [...] The channel depicted in [...] is equal to $\mcG_{(0,3)}\mcG_{(3,3)}$
\begin{equation}
\mcL=\frac{\gamma_{(0,3)}\left(\one\otimes\sigma_z\rho\one\otimes\sigma_z -\rho \right)}{2}+\frac{\gamma\left(\sigma_{\vec \alpha}\rho \sigma_{\vec \alpha} -\rho \right)}{2}
\end{equation}

In addition to decoherence processes, PCE channels can also be described with collision models, 
\begin{itemize}
