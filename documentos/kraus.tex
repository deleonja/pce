...
some Markovian processes. In this section we prove what in fact they are, and
...
PCEGs \janote{introducimos esta notación antes?} and their \textit{Kraus representation} (or operator-sum representation)
...
invariant~\cite{nielsen_chuang_2011}. \ddnote{agrego lo de los flips aqui}{\blue Notice that according to Kraus operators, the generators $\mcG_{\vec \alpha}$ are $N$-qubit flip channels with flip probability of $1/2$, where the joint flip is $\rho \mapsto \sigma_{\vec \alpha} \rho \sigma_{\vec \alpha}$. In fact, tracing out all particles except particle $k$ gives the well known qubit flip channels, \ie{} $\tr_{\not k} \mcG_\valpha=\mcG_{\alpha_k}$ (see~ \eref{eq:generator:individual:qubit}). And more generally, tracing out $m$ particles leaves a $N-m$ particles flip channel.}
...
by Kraus operators  \janote{in} \eref{eq:kraus:PCE} is a PCE channel. Moreover, one can notice that, 
...
then generalizing to more general channels. 
