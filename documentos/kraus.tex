...
experimental~\cite{PhysRevA.67.042322} settings and are archetypical in decoherence dynamics. Moreover, these processes lead to a monotonically \afnote{monotonic (continuous)} lose of information~\cite{breuerreview} and describe non invertible channels in the asymptotic limit $t\to \infty$ (this can be seen from the monotonically \afnote{(continuous)} decreasing of the determinant, see Ref.~\cite{cirac}).
...
some Markovian processes. In this section we prove what \afnote{that} in fact they are, and
give two examples of Markovian implementations.
%
The first
one consist in identifying each \pce{} channel as a fixed point of some pure
dissipative process. The second, in identifying each \pce{} channel as fixed
points of some memory-less collision model \afnote{The first of them consists in identifying each \pce{} channel as a fixed point of some pure dissipative process, and in the second implementation we relate each \pce{} channel to fixed
points of some memory-less collision model }.
...
PCEGs \janote{introducimos esta notación antes?} \afnote{(acabo de colocarla en el 3er parrafo de la sección de generadores)} and their \textit{Kraus representation} (or operator-sum representation)
...
Inspection of the Kraus operators of two qubit PCEGs leads to
the ansatz that the Kraus operators for the generator of 
$\mcG_{\vec \alpha}$ are 
\afnote{By inspection, it is easy to prove that two qubit PCEGs leads to
the ansatz that the Kraus operators for the generator of 
$\mcG_{\vec \alpha}$ are}
\begin{equation}
...
\label{eq:kraus:PCE}
\end{equation}
since the Kraus operators corresponding to one \afnote{a} single qubit PCEs are 
$\{\openone/\sqrt2, \sigma_\alpha/\sqrt2\}$,
corresponding to the operation that leaves the component $\sigma_\alpha$ \janote{$r_0$ and $r_\alpha$}
invariant~\cite{nielsen_chuang_2011}. \ddnote{agrego lo de los flips aqui}{\blue Notice that according to Kraus operators, the generators $\mcG_{\vec \alpha}$ are $N$-qubit flip channels with flip probability of $1/2$, where the joint flip is $\rho \mapsto \sigma_{\vec \alpha} \rho \sigma_{\vec \alpha}$. In fact, tracing out all particles except particle $k$ \afnote{...except the $k-$th one} gives the well known qubit flip channels, \ie{} $\tr_{\not k} \mcG_\valpha=\mcG_{\alpha_k}$ (see~ \eref{eq:generator:individual:qubit}). And more generally, tracing out $m$ particles leaves a $N-m$ particles flip channel \afnote{with weight $1/2$}.}

We shall first show that the Kraus operators in \eref{eq:kraus:PCE} produce \janote{are indeed} a PCE. 
Notice that 
% \begin{equation}
$\sigma_\alpha \sigma_\beta \sigma_\alpha =a_{\alpha \beta} \sigma_\beta$, see~\Eref{eq:1},
% \label{}
% \end{equation}
which in turn imply \afnote{implies} that 
\begin{equation}
