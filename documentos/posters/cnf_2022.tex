...
\renewcommand{\indent}{\hspace*{25pt}}
...
\section{Resumen}
\indent Procesos de disipación, como la decoherencia de los sistemas cuánticos, pueden describirse con la teoría de los canales cuánticos. 
%No obstante, sigue siendo una tarea difícil entender el conjunto de todos los canales cuánticos, especialmente para sistemas de muchas partículas. 
En ese contexto, proponemos los mapeos \textit{Pauli component erasing} (PCE), que preservan o borran por completo las componentes de la matriz de densidad de un sistema de muchos qubits. 
Mostramos que el subconjunto de mapeos PCE que describen evoluciones físicas, es decir, los \textit{canales cuánticos PCE}, se corresponden con subespacios vectoriales.
%Para aquellos que son canales cuánticos mostramos su correspondencia con subespacios vectoriales y así hacemos su caracterización. 
Finalmente, derivamos los generadores de este subconjunto a partir de su estructura de semigrupo y, con esto, determinamos implementaciones físicas para conectar a los canales cuánticos PCE con procesos Markovianos.

% \section{Introducción}
\section{Motivación}
\indent 
%Los estados de una partícula de dos niveles, un qubit, pueden visualizarse geométricamente en la esfera de Bloch. 
Algunos procesos de decoherencia de 1 partícula de 2 niveles se describen como proyecciones de la esfera de Bloch. Sin embargo, no todas las proyecciones describen evoluciones físicas [vea la Fig. \ref{fig:projections}].
%\begin{figure}
%    \centering
...
\node at (-6, 0)  {%
%\includegraphics[width=.2\columnwidth,page=1]{../images/blochs-crop}%
\includegraphics[width=7cm,page=2]{../images/blochs-crop}%
};
\draw [-stealth,line width=8pt] (-1,0) -- (1,0);
%\draw [-{stealth[length=3mm, width=3mm]},line width=3pt] (1.4,0) -- (2.7,0); 
\node at (0.,1.5) {\color{green} \Huge\cmark};
\node at (0.,-8.5) {\color{red} \Huge\xmark};
%\draw [-{stealth[length=3mm, width=3mm]},line width=3pt] (1.4,-2.8) -- (2.7,-2.8); 
\draw [-stealth,line width=8pt] (-1.2,-10) -- (1.2,-10); 
\node at (6,  0)  {%
\includegraphics[width=7cm,page=3]{../images/blochs-crop}%
% \only<2->{\includegraphics[width=.2\textwidth,page=2]{../images/blochs-crop}}%
};
\node at (6,  -8)  {%
\includegraphics[width=7cm,page=4]{../images/blochs-crop}%
};
\node at (-6,  -8)  {%
\includegraphics[width=7cm,page=5]{../images/blochs-crop}%
};
\end{tikzpicture}
...
\end{center}
\caption{No todas las proyecciones de la esfera de Bloch describen evoluciones físicas.}
\label{fig:projections}
\end{figure}
...
Un canal cuántico es un mapeo de matrices de densidad a matrices de densidad. Es decir, $\rho\mapsto\mathcal{E}(\rho)$, donde $\mathcal{E}$ es un mapeo completamente positivo que preserva la traza de $\rho$.
...
\section{Planteamiento del problema:\\mapeos PCE}
...
\indent Definimos a un mapeo \textit{Pauli component erasing} (PCE) como un mapeo que actúa sobre $\rho$, de la forma \eqref{eq:rho}, como
\begin{align}\label{eq:pce:N:qubits}
    r_{\valpha}\mapsto\tau_{\valpha} r_{\valpha},\quad \tau_{\valpha}=0,1.
\end{align}
Para que un mapeo PCE preserve la traza de $\rho$ por definición imponemos $\tau_{\vec 0}=1$. Nos vamos a referir a un mapeo de la forma \eqref{eq:pce:N:qubits}, pero sin la restricción sobre los valores de $\tau_{\valpha{}}$, como un mapeo diagonal de Pauli. En la Fig. \ref{fig:2:qubits:diagrams} introducimos diagramas para representar a los mapeos 
PCE de 1 y 2 qubits. 
%============================================
\vskip2ex
\begin{figure}
\centering
\includegraphics[height=12cm]{../images/one_and_two_qubit_PCE.pdf}
\vspace*{1ex}
...
\end{figure}
\indent En general, un mapeo PCE [vea Eq. \eqref{eq:pce:N:qubits}] no es completamente positivo y, por consiguiente, no todos los mapeos PCE son canales cuánticos. 
Para determinar si un mapeo PCE es completamente positivo investigamos la diagonalización de la matriz de Choi de un mapeo diagonal de Pauli.

% \section{Consideraciones matemáticas} d
\section{Diagonalización de la matriz de Choi $\pmb{\mathcal{D}}$}
...
\begin{figure}
\centering
\includegraphics[height=6.5cm]{img/connection_a_generators.pdf}
