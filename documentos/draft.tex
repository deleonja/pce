...
setting,  is still an ongoing difficult task. We propose a family of quantum
maps that generalize total 


We generalize the notion that \afnote{an (any?) operation that collapeses the Bloch sphere into a disk with unity radius does not correspond to a quantum channel, ...} collapsing the Bloch sphere into a disk with unity radius is not a quantum channel, to the case of an arbitrary number of qubits. This is, we study and characterize all generalized idempotent Pauli channels to the case of $N$ qubits. The generalization is obtained by extending Pauli matrices to Pauli strings \afnote{(Tal vez no sea necesario hablar de Pauli strings aquí)}. \afnote{We present a full characterization of completely positive maps of this type by interpreting channels' matrices as finite vector spaces.} We give a complete characterization of completely positive maps of this type by means of interpreting channels as finite vector spaces. Moreover, we show that the obtained family of channels is a semigroup and derive its generators. We use this simple structure to determine physical implementations and connect the obtained family of channels with Markovian processes.
\end{abstract} % }}}
%%
