...
\cpnote{Notas, no entiendo que es 3+3. Ceo qe no es suma de Klein tampoco 
por un aparente contraejemeplo: 
$3=\Sigma(0) = \Sigma(0+0) = \Sigma(0)+\Sigma(0) = 3+3 = ??$}
\cpnote{Este es un mapeo que depende de $k$, por lo que propondria ponerle
o un subindice o un superindice $(k)$. }
\cpnote{La simetría clasifica también al generador!! Quizá podemos quitar el miniapendice
que había escrito}
...
\begin{equation}
\Sigma: 0\mapsto3,\quad3\mapsto0,\quad1\mapsto2,\quad2\mapsto1
\end{equation}
Indeed, any generator is a subspace containing half the elements of the full vector space.  
The map $\Sigma$ has the following properties
\begin{enumerate}
\item $\Sigma(\vec\alpha)+\Sigma(\vec\beta)=\vec\alpha+\vec\beta$.
\item $\Sigma(\vec\alpha)\neq\vec\alpha$
\end{enumerate}
From the first property, we now obtain
\begin{equation}
\vec\alpha=\Sigma(\vec\alpha)+\Sigma(\vec0)
\end{equation}
