...
quantum channels, how to label each element of this set according to its local-action to 
every qubit in the system, and, finally, we  	will discuss a symmetry of
...
in the system, and thus be denoted as $\mcG_{\vec\alpha}$. First, let us denote
the four 1-qubit PCE generators as $\mcG_{\alpha}$, with $\alpha=0,1,2,3$.
The first being the trivial generator and the remaining the channels that 
map the Bloch sphere to each of the Cartesian axes.
%PCE generator may be identified with a unique vector index
...
%If one sets all indices in $\vec\alpha$ to zero except for the $k$-th index.
\janote{A rigorous explanation of why every $\vec\alpha$ is corresponded
to every generator.}
...
See in \Fref{fig:appex:twoQ_PCEGs} the notation of  two-qubit PCE generators.
%See, for example, non-trivial generators in subfigures (c), (d) and (e) of
%\Fref{fig:examplesPCE} are denoted $\mcG_{(1,3)}$, $\mcG_{(3,2)}$ and
%$\mcG_{(2,2)}$, respectively.
...
The same symmetry can be identified for both the PCE generators $\pceg$ and
matrix $A$ the diagonalizes PCE map's Choi matrix $\mcD_N$.
%PCE generators $\pceg$ obey a symmetry
%that defines uniquely every PCE generator. 
For a PCE generator the symmetry is encoded in the array of $\taus$, and 
for $A$ in its rows and columns.\janote{aquí voy}
The coefficients  $\{\taus\}$ of every 
