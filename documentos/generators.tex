...
\janote{PCE quantum channel generators and a connection between
them and and $A$, the matrix that diagonalizes the Choi matrix $\mcD_N$ 
of a PCE map.}
\sout{both PCE quantum channel generators and $A$, the matrix that diagonalizes
the Choi matrix $\mcD_N$ of a PCE map.}
\janote{Al final de la sección no discutimos la simetría para $a$, sólo
comentamos que están conectados los PCEGs y la $a$}
...
Consider the map $\Sigma^{(k)}$ that reflects \janote{a} \sout{an} multi-index $\valpha$ with 
...
\janote{Finally,} it is worth pointing out that $\san$ (and thus $a$) [see \eref{eq:san}] 
...
$-1$\janote{s} with $0$\janote{s}. 
The proof \janote{of this connection between PCEGs and $A$} is given in \appref{sec:2_qubits}\janote{,} and in \fref{fig:A_and_PCEGs_connection} we illustrate
this connection for the single\janote{-}qubit case.
