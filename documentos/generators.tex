...
By standard theorems of linear algebra, any subspace $W$ [see. \Eref{eq:19}] can be extended to a
maximal (non-trivial) subspace of dimension $2N-1$ by adjoining appropriate
additional basis elements. This can clearly be done in different ways. 
We therefore arrive to the set of maximal extensions of $W$,
where every maximal subspace corresponds to a PCE quantum channel that 
preserves $2^{2N-1}$ (half the) Pauli components $\paulicomponents$ 
of density matrix $\rho$. The
intersection of all the elements of this set reduces to $W$ itself, meaning 
that all PCE's can be obtained as intersections of maximal PCE's, the
PCE generators $\mcG$. Intersections of the maximal subspaces translates into 
compositions of the generators $\mcG$ to obtain any PCE.
Compositions of generators $\mcG$ can be understood intuitively from PCE diagrams
in \Fref{fig:examplesPCE}; the composition of any two 2-qubits PCE generators 
