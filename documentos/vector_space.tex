...
\cpnote{Ojo, cambie el nombre de la seccion. Pendiente de sus comentarios}
% \ddnote{propongo channels as vector spaces como titulo de seccion, pues lo
% actual se entiende mas en el sentido estandar: canales como miembros de un
% espacio vectorial, que tambien es cierto, pero no es el punto aqui}
% \cpnote{Falta lo de las clases de equivalencias, pero aun no encuentro el
% espacio. Quizá cuando haga falta se puede poner}
% \janote{Se me hace que podemos agregar algo al final, ya que es la parte en la que
% se hablan de la regla $2^k$ y regla espejo. No tengo una idea clara, pero si te 
% parece una buena idea podría elaborar más cómo meterlo ahí}
% \par
...
\eref{eq:N_qubits_rho}. \cpnote{Some established facts about vector spaces will allow
us to draw conclusions for PCE quantum channels.}
\janote{Prefiero esta frase:}
\janote{Some established facts about vector spaces will allow
...
of a PCE map can be recast as determining which vectors $\vtau$ are mapped via
$\san$
% \begin{equation}
...
Other two simple but crucial observations are the following. 
For valid quantum channels it holds 
% \begin{equation}
% % \sum_{
% \end{equation}
...
$\{0,1\}$.
We notice that the Klein group described in \fref{tab:1} is actually isomorphic
...
whenever the $\vec\gamma$ belongs to a given vector subspace of the set of
all indices, then the vector $\vec\tau$ indeed has an image
which is a vector
...
from the solution having $\tau_{\vec0}=1$, with everything else $0$. We may
then successively switch $\tau_{\vec\alpha}$'s
\janote{agregué el subíndice $\vec\alpha$} 
to $1$ for various values of $\valpha$,
taking care immediately to set equal to one the components of $\vec\tau$
\janote{puse la $\tau$ como vector}
that
...
\end{center}
\caption{
\cpnote{Revisar, solo tiene una iteracion por parte mia}
Examples of diagrams for several PCE quantum channels. In
 (a) the totally depolarizing channel is represented whereas
in (b) a PCE channel that preserves four components.  (c), (d)
and (e) show three generators \cpnote{Quizá poner los nombres de los generadores acá}; the combination of any two of them
yields the channel in (b). (f) represents the identity map. 
% 
% \cpnote{Mejorar el caption cuando me ponga de acuerdo con la gente}
...
% **********************************
% 
% \cpnote{Dejar que la raza elija uno de los dos parrafos anteriores}\janote{El
% primero (simón, cambié de opinión)} \par 
...
\end{equation}
which suggest a relation  between individual channels that
