...
\cpnote{Quiza falta lo de las clases de equivalencias}\janote{Pienso que sí porque
puede y debería ser útil para la sección siguiente de generadores.}\par
...
Pauli channel, as in \eref{eq:definition:Pauli}. \janote{por qué diagonalizar al 
...
$\lambda_\valpha=0$ for all $\valpha\in \Omega$\janote{Qué es $\Omega$?}, as each member of the sum \janote{is}
...
$\sum_{\vec\beta}\san_{\vec\alpha\vec\beta}\lambda_{\vec\beta}=2^N$\janote{la resta no es al revés?}
...
% \sa_{\alpha_k\beta}=1.
% \end{equation}
This $\vec\beta$, as is readily verified, belongs to $\Phi(\vec\alpha)$, so that \eref{eq:13}
indeed holds for this $\vec\beta$. On the other hand, it is then readily verified that, for this
$\vec\beta$, \eref{eq:13} reduces to 
...
\label{eq:14}
\end{equation}
for all $\beta$ such that $\sa_{{\alpha_k}\beta}=1$ . One can verify, by working out the different cases, 
...
\cpnote{Dejar que la raza elija uno de los dos parrafos anteriores} \par 
