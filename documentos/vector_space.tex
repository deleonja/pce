...
\cpnote{Quiza falta lo de las clases de equivalencias}\par
...
This is a remarkable equation, as it provides a method to diagonalize any diagonal 
...
Condition (\ref{eq:coneccion:dos:indices:A}) therefore connects three multi-indices, $\vec\alpha$,
...
% \cpnote{Creo que aca podemos referenciar la figura 2c y mostrar como
% funcionaria que no es cerrada bajo la suma}
...
$\valpha^{(1)} + \valpha^{(2)} = (1,2)$, which is not preseverd and thus this 
...
depolarizante y el subespacio mas grande es la identidad} \par
...
We can then see that all PCEs preserve $2^K$ components. In fact, with the above 
...
elements of the vector space. This means that any PCE will preserve $2^K$ 
...
seen earlier that
\cpnote{Why is the dimension $2N$? I can work it out, but just after adding one 
by one elements}
\begin{equation}
W=\left\{
...
\cpnote{Dejar que la raza elija uno de los dos parrafos anteriores}
 % }}}
...
\end{equation}
which suggest an relation  between individual channels that for the time being
