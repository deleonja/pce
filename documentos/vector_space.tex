...
\cpnote{ojo cno factores globales, depende de la definicion de la $a$} 
% (so $A = A^{-1}/2^N$) 
\begin{equation}
A^{-1} = \frac{1}{4^N} A
\end{equation}
we can directly invert \eref{eq:lambda:is:A:tau} to obtain (in matrix form)
\begin{equation}
...
\label{eq:tau:is:A:p}
\end{equation}
\cpnote{me da diferente el factor global que al resto de la banda}
...
\begin{equation}
\sum_\valpha \lambda_\valpha = 4^N,
\label{eq:sum:alpha:is:4N}
\end{equation}
